...
export function CompensationDialog({
  ...
  const [type, setType] = useState<CompensationType>(
    creator.compensation?.type || "PER_VIDEO"
  );
  const [perVideoRate, setPerVideoRate] = useState(
    creator.compensation?.perVideoRate?.toString() || ""
  );
  const [customAmount, setCustomAmount] = useState(
    creator.compensation?.customAmount?.toString() || ""
  );
  const [customNote, setCustomNote] = useState(
    creator.compensation?.customNote || ""
  );
  const [isFixedMonthly, setIsFixedMonthly] = useState(
    creator.compensation?.isFixedMonthly ?? false
  );
  ...
  useEffect(() => {
    setType(creator.compensation?.type || "PER_VIDEO");
    setPerVideoRate(creator.compensation?.perVideoRate?.toString() || "");
    setCustomAmount(creator.compensation?.customAmount?.toString() || "");
    setCustomNote(creator.compensation?.customNote || "");
    setIsFixedMonthly(creator.compensation?.isFixedMonthly ?? false);
    setError("");
  }, [creator]);
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch(`/api/creators/${creator.id}/compensation`, {
        method: "PUT",
  ...
  return (
    <Dialog open={open} onClose={onClose} title={`報酬設計 — ${creator.name}`}>
